...
export default function Fake() {
    ...
    useEffect(() => {
        setAllProducts(fakepro)
        setProducts(fakepro)
        setCategories([...new Set(fakepro.map(item => item.category))])
    }, [fakepro])
    ...
    return (
        <div className='Api container my-5'>
            ...
            <div className="mb-3 d-flex gap-2 flex-wrap justify-content-center">
                {categories.map(cat => (
                    <button key={cat} className="btn btn-primary active" onClick={() => filterByCategory(cat)}>
                        {cat}
                    </button>
                ))}
                {showAllBtn &&
                    ...
                    products.map((val) => (
                        <div className='col-lg-3 col-md-6 col-sm-12' key={val.id}>
                            <div className="card p-3 bg-white">
                                <Link to={`/product/${val.id}`} className="img d-flex align-items-center justify-content-between">
                                    <img src={val.image} style={{ height: '200px' }} className='img' alt='' />
                                </Link>
